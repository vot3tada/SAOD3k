...
void ThreadWriterC(std::list<std::future<std::string>>* queue, std::string nameOutFile, bool* stop)
{
...
void ThreadWriterD(std::list<char>* queue,std::string nameOutFile, bool* stop)
{
    std::ofstream fout(nameOutFile, std::ios::out | std::ios::binary);
    while (!*stop || !queue->size() == 0)
    {
        if (queue->size()==0) continue;
...
void Compress(std::string nameInFile, std::string nameOutFile)
{
    ...
    status.push_front(std::async(std::launch::async, ThreadWriterC, &queue2, nameOutFile, &stop));
    while (!fin.fail())
    {
        char* text = new char[len];
        fin.read(text, len);
...
void Decompress(std::string nameInFile, std::string nameOutFile)
{
    ...
    fin.open(nameInFile, std::ios_base::in | std::ios_base::binary);
